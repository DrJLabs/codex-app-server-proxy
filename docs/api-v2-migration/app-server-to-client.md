...
14) **Final envelope (tool-block parsing still active in openai-json):** Aggregated tool calls are combined with parsed `<tool_call>` blocks and included as function call items in the responses envelope. This merge is **independent of output mode**; openai-json does not disable `<tool_call>` parsing. (`src/handlers/responses/nonstream.js`, `src/handlers/responses/native/envelope.js#buildResponsesEnvelope`, `src/handlers/responses/tool-call-parser.js`)
