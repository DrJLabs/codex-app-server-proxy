...
12) **Tool-call parsing for text (legacy compatibility, still active in openai-json):** When tools are enabled, a `<tool_call>` parser processes textual deltas and converts them into structured function calls. This parsing is **not gated by output mode**; it runs even in openai-json. (`src/handlers/responses/stream-adapter.js`, `src/handlers/responses/tool-call-parser.js`)
...
14) **Final envelope (tool-block parsing still active in openai-json):** Aggregated tool calls are combined with parsed `<tool_call>` blocks, deduplicated by tool name and normalized JSON arguments, and included as function call items in the responses envelope. This merge is **independent of output mode**; openai-json does not disable `<tool_call>` parsing. (`src/handlers/responses/nonstream.js`, `src/handlers/responses/native/envelope.js#buildResponsesEnvelope`, `src/handlers/responses/tool-call-parser.js`)
...
1) **Double-source tool calls can duplicate results.** If upstream emits both structured tool calls and textual `<tool_call>` blocks for the same invocation, tool calls can be duplicated. This is mitigated by deduping parsed calls against aggregated calls (name + normalized JSON arguments). (`src/handlers/responses/nonstream.js`)
2) **Tag mismatch across endpoints.** Chat tooling expects `<use_tool>` while responses parsing expects `<tool_call>`, making mixed outputs brittle during migration and increasing the chance of missed tool parsing. This mismatch **still matters in openai-json**, because responses continues to parse `<tool_call>` blocks even when output mode is openai-json. (`src/handlers/chat/tool-output.js`, `src/handlers/responses/tool-call-parser.js`)
...
2) **Deduplicate tool calls when merging sources.** When both structured tool calls and textual tool-call blocks are available, dedupe by name + arguments before emitting the final responses envelope.
